    ...
    public class FileStoremanWS : DatabaseAccessService, IFileStoremanWS
    {
        ...
        public int StoreTrialFile(int trialID, string path, string description, string filename)
        {
            ...
            */
            if (filename.Normalize().Contains('\\') || filename.Normalize().Contains('/'))
            {
                faultMsg = "Subdirectory symbol detected in: '" + filename + "'. Must be a simple file name.";
                FileAccessServiceException exc = new FileAccessServiceException("Wrong file name", faultMsg);
            ...
            {
                FileAccessServiceException exc = new FileAccessServiceException("unknown", "Update failed");
                throw new FaultException<FileAccessServiceException>(exc, "File acccess invocation at DB side: " + ex.Message, FaultCode.CreateReceiverFaultCode(new FaultCode("StoreTrialFile")));
        ...
        public int CreateSessionFromFiles(string path)
        {
            ...
            try
            {
                ...
                foreach (FileInfo fi in di.GetFiles("????-??-??*-S??*.??*", SearchOption.TopDirectoryOnly))
                {
                    ...
                    if (Regex.IsMatch(fi.Name, @"(\d{4}-\d{2}-\d{2}-[AB]\d{4}-S\d{2}(-T\d{2})?(\.\d+)?\.(asf|amc|c3d|avi|zip|vsk|mp))|(\d{4}-\d{2}-\d{2}-S\d{4}(-T\d{4})?\.(png|xml|dat|c3d))"))
                    {
