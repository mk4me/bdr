...
UAP as (select * from Performer Performer where exists (select * from Konfiguracja_performera KP where exists (select * from UAS where UAS.IdSesja = KP.IdSesja) )),
UAPC as (select * from Konfiguracja_performera PerformerConf where exists(select * from UAS where UAS.IdSesja = PerformerConf.IdSesja)),
UAUG as (select * from Grupa_uzytkownikow UserGroup 
	where exists(select * from Uzytkownik_grupa_uzytkownikow ugu where ugu.IdUzytkownik = dbo.identify_user(@user_login) and ugu.IdGrupa_uzytkownikow = UserGroup.IdGrupa_uzytkownikow))
select
(select
(select 
	IdSesja as SessionID,
...
(select 
 ...
 (select 
	...
	from UAUG UserGroup FOR XML AUTO, TYPE
 )MyUserGroupMemberships
 for XML RAW ('ShallowCopyBranches'), TYPE;
go

-- exec get_shallow_copy 'habela', '2013-10-10 00:00:00.000';

exec get_shallow_copy 'habela', '2013-12-12 00:00:00.000'
...
select 
	...
	where exists(select * from Uzytkownik_grupa_uzytkownikow ugu where ugu.IdUzytkownik = dbo.identify_user(@user_login) and ugu.IdGrupa_uzytkownikow = UserGroup.IdGrupa_uzytkownikow) FOR XML AUTO, TYPE) UserGroup
for XML RAW('MyUserGroupMemberships'), TYPE;
go
