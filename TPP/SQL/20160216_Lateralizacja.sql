...
alter table Wizyta add Lateralizacja tinyint;

update Kolumna 
set PozycjaDomyslna = PozycjaDomyslna + 1
where PozycjaDomyslna > (select PozycjaDomyslna from Kolumna where Encja = 'W' and Nazwa = 'PoprawaPoLDopie');
go

insert into Kolumna ( PozycjaDomyslna, Encja, Nazwa) 
select PozycjaDomyslna+1, 'W', 'Lateralizacja' from Kolumna where Encja = 'W' and Nazwa = 'PoprawaPoLDopie';
go
